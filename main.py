...
def activate_vip():
    if 'user_id' not in session:
        return redirect(url_for('login'))  # Preusmeri, če uporabnik ni prijavljen

    user = User.query.get(session['user_id'])
    if user:
        user.is_vip = True  # Nastavi VIP status uporabnika
        db.session.commit()  # Shrani spremembe v bazo
        session['is_vip'] = True  # Dodaj tudi v sejo
        return redirect(url_for('leaderboard'))  # Preusmeri na leaderboard (lahko spremeniš stran, kamor ga boš poslal)
